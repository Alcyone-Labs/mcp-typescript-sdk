...
describe("Transport resumability", () => {
  ...
  beforeEach(async () => {
    ...
    mcpServer.tool(
      ...
      "Sends multiple notifications over time",
      {
        ...
        interval: z
          .number()
          .describe("Interval between notifications in ms")
          .default(50),
      },
  ...
  it("should resume long-running notifications with lastEventId", async () => {
    ...
    // Close transport and client, and properly await the request promise rejection
    await transport1.close();

    // Wait for the tool promise to reject due to connection closure
    try {
      await toolPromise;
    } catch (err) {
