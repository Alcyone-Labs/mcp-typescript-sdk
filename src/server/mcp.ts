...
export class McpServer {
          ...
          .map(([name, tool]): Tool => {
            const toolDefinition: Tool = {
              ...
              inputSchema: tool.inputSchema
                ? (safeToJSONSchema(tool.inputSchema) as Tool["inputSchema"])
                : EMPTY_OBJECT_JSON_SCHEMA,
              annotations: tool.annotations,
            };

            if (tool.outputSchema) {
              toolDefinition.outputSchema = safeToJSONSchema(
                tool.outputSchema,
  ...
  private async handlePromptCompletion(
    ...
    const suggestions = await field.complete(
...
function safeToJSONSchema(schema: any): any {
  try {
...
function isZodRawShape(obj: unknown): obj is ZodRawShape {
  ...
  const filteredValues = Object.entries(obj as object)
...
function promptArgumentsFromSchema(
  ...
  return Object.entries(schema.shape).map(
    ([name, field]): PromptArgument => ({
      ...
      required: !((field as any).isOptional?.() ?? false),
    }),
  );
}
